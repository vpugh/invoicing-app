  ...
  methods: {
    saveTransaction() {
      ...
      if( name.length != 0 && price > 0){
        ...
        this.txn_names.push({ name: name});
        this.txn_prices.push({ price: price});
        this.nextTxnId++;
        this.calcTotal();
        // clear their values
    ...
    //   });
      console.log(this.txn_names);
      console.log(this.txn_prices);
      formData.append("name", this.invoice.name);
      formData.append("txn_names", this.txn_names);
      formData.append("txn_prices", this.txn_prices);
